                          ...
                          uint8_t* greyscale_result)
{
    ...
    {
        greyscale_result[n++] = (uint8_t) round (
 ...
 */
static void adjust_brightness (uint8_t* input_image, size_t img_size, int16_t brightness)
{
...
compute_standard_deviation (const uint8_t* input_image, size_t img_size, double average)
{
    ...
    return sqrt (sum / (double) (img_size));
}
...
static void adjust_contrast (uint8_t* input_image, size_t img_size, float k)
{
    ...
    for (size_t i = 0; i < img_size; i++)
    {
        ...
        // clamping
        double unclamped = round ((input_image[i] * k_div_sigma_normalized
        ...
        {
            input_image[i] = (uint8_t) unclamped;
        }
    }
}
/**
 ...
 */
void brightness_contrast_V1 (const uint8_t* img,
                             size_t width,
                             size_t height,
                             float a,
                             float b,
                             ...
                             uint8_t* result)
{
    ...
    if (brightness != 0)
    {
        adjust_brightness (result, img_size, brightness);
    }
